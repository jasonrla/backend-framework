...
describe('Payout cancelled', () => { //test10
    ...
    data.forEach(({ username, password, amount, merchantCountry, merchantCurrency, merchantAmount, provider, xbFeeRateExpected }) => {
        ...
        test('Send payout "Cancelled" event', async () => {
        ...
        test('Check "Cancelled" row is created in transaction_log', async () => {
            ...
            const queryParams = [ customerId, "cancelled", externalReference, currentMonth, currentYear ];
            
            const result = await query('ledger', sqlQuery, queryParams);
            
            expect(result).toHaveLength(1);
            expect(result[0].customer_id).toBe(customerId);
            expect(result[0].situation).toBe("cancelled");
